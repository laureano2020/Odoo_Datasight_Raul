...
odoo.define('spiffy_theme_backend.MenuJs', function(require) {
    ...
    var {
        NavBar
    } = require("@web/webclient/navbar/navbar");
    var {
        patch
    } = require("web.utils");
    const {
    ...
    patch(NavBar.prototype, "theme_backend.MenuJs", {
        async setup(parent, menuData) {
            ...
            $(document).on('click', 'body:not(.top_menu_vertical_mini) .o_navbar_apps_menu .main_link', function(ev) {
                self._ShowCurrentMenus(ev)
            });
            $(document).on('click', 'body.top_menu_vertical_mini .o_navbar_apps_menu .main_link', function(ev) {
                self._ShowCurrentMenusNew(ev)
            });
            ...
            $(document).on('shown.bs.modal', '#search_bar_modal', function(ev) {
                self._all_apps_records_data(ev)
            });
            // 14 end
            $(document).on('shown.bs.modal', '#search_bar_modal', function(ev) {
            ...
            // 14 start
            $(document).on('click', '.form-check-input', function(ev) {
                self._showSearchbarModalOption(ev)
            });
            // 14 end
            ...
            $(document).on('click', '.header_menu_right_content_toggler .bookmark_panel_toggle', function(ev) {
                self._ToggleBookmarkPanel(ev)
            });
        ...
        _all_apps_records_data: function() {
            var self = this;
            ajax.jsonRpc('/get/records/global/search', 'call', {}).then(function(rec) {
        ...
        _showSearchbarModalOption: function() {
            ...
            console.log("Selected searchOption value:", searchOption);
        ...
        _showSearchbarRecord: function () {
            console.log("change records selection");
            this.$search_modal_results.empty();
            this.$search_modal_Noresults.toggleClass('d-none', true);
            this.$search_modal_input.val("");
        },
        ...
        _showSearchbarMenu: function () {
            console.log("change menu selection");
            this.$search_modal_results.empty();
            this.$search_modal_Noresults.toggleClass('d-none', true);
            this.$search_modal_input.val("");
        },
        ...
        _searchPages: function() {
            ...
            if (searchvals_option === "menu") {
                ...
                options.each(function() {
                    ...
                    ajax.jsonRpc('/get/records/data', 'call', {
                        'model': model,
                        'id': id,
                        ...
                        if (rec) {
                            ...
                            search_modal_results.html(
                                core.qweb.render("spiffy_theme_backend.RecordSearchResults", {
