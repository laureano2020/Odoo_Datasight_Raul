...
const userMenuRegistry = registry.category("user_menuitems");
    ...
    var { onMounted, onWillStart, useState, useChildSubEnv, onWillUpdateProps } = owl;
    ...
    patch(ListRenderer.prototype, "spiffy_theme_backend.ListRenderer", {
      setup() {
          ...
          // load attachment after any filter or page change
          ...
          async _performUpload({ files }) {
              ...
              for (const file of files) {
                  ...
                  try {
                      ...
                      if (session.bg_color) {
                        const response = await (composer || thread).messaging.browser.fetch('/app/attachment/upload', {
                            method: 'POST',
                            body,
